...
plt.rcParams['legend.fontsize'] = 8 # 'large'
...
def spectrogram_overlay(i, range=[2.75, 5.25], save_plot=True, show_plot=True, figsize=(3.404, 5.295),
    ...
    leg0 = ax0.legend(shadow=True, numpoints=1, loc='lower right')
    leg0.get_frame().set_linewidth(0.4) # 1.0
    ...
    plot_stft(stft, t, f, cmap='gray_r', range=range)
    # ax = plt.gca()
    ((df.f13_theory+frf)/1e3).plot(ax=ax1, label=r'$f_{\mathrm{rf}} + f_{13}$', c=c13) #\,\mathrm{ (theory)}$')
    ((df.f12_theory+frf)/1e3).plot(ax=ax1, label=r'$f_{\mathrm{rf}} + f_{12}$', c=c12) #\,\mathrm{(theory)}$')
    # ((df.f12+frf)/1e3).plot(ax=ax, yerr=all_u_df[i]/1e3, c='g', ecolor='g', label=r'$f_{12}\,\mathrm{(expt.)}$')
    ((df.f23_theory+frf)/1e3).plot(ax=ax1, label=r'$f_{\mathrm{rf}} + f_{23}$', c=c23) #\,\mathrm{ (theory)}$')
    # ((df.f23+frf)/1e3).plot(ax=ax, yerr=all_u_df[i]/1e3, c='c', ecolor='c', label=r'$f_{23}\,\mathrm{(expt.)}$')
    plt.axhline(frf/1e3, ls='--', dashes=(2,2), c='orange', label=r'$f_{\mathrm{rf}}$')
    plt.xlabel('time (s)')
    plt.ylabel('frequency (kHz)')
    ...
    leg1 = ax1.legend(shadow=True, numpoints=1, loc='upper right')
    leg1.get_frame().set_linewidth(0.4) # 1.0
    ...
    leg2 = ax2.legend(shadow=True, numpoints=1, loc='lower right')
    leg2.get_frame().set_linewidth(0.4) # 1.0
    plt.subplots_adjust(hspace=.1)
    plt.tight_layout()

    if save_plot:
        plt.savefig('plots/spectrogram/{:}_spectrogram_.png'.format(ds))
        plt.savefig('plots/spectrogram/{:}_spectrogram_.pdf'.format(ds))
    if show_plot:
        plt.show()

    # Draw inset over 3.2mG range
    plt.figure(figsize=(1.5,0.7186))
    ax = plt.subplot(111)
    plt.errorbar(dB_mG, subdf.f12.values/1e3, xerr=u_dB_mG, yerr=u_subdf.f12.values/1e3, label=r'$f_{12}$ (expt.)', fmt='o', c=c12, ecolor=c12)
    plt.plot(1e3*dB_p, f12_p/1e3, label='$f_{12}$ (theory)', c=c12)
    ...
    plt.tight_layout()
    if save_plot:
        plt.savefig('plots/spectrogram/{:}_spectrogram_inset_.png'.format(ds))
        plt.savefig('plots/spectrogram/{:}_spectrogram_inset_.pdf'.format(ds))
    if show_plot:
        plt.show()