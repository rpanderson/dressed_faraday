...
\begin{itemize}
    ...
    \item Mention relation bands signal strength to populations of dressed eigen states. We have done this correct?
\end{itemize}
...
\begin{itemize}
    ...
    \item The Faraday signal we detect is proportional to $\expect{\hat{F}_x}$ in the laboratory frame, which is given by $\bra{\psi}\hat{S}^{\dagger} \hat{F}_x \hat{S} \ket{\psi}$ where $\hat{S} = \exp(-i \omega_{\text{rf}} \hat{F}_z t)$. For an initially polarized $\ket{\psi(t=0)} = \ket{m=1}$ state driven on resonance, we get
    \begin{align*}
        \expect{\hat{F}_x}_{\text{lab}} &= -\frac{\Omega}{\omega_D} \cos(q_D t) \sin(\omega_D t) \sin(\omega_{\text{rf}} t) - \frac{\Omega q_D}{\omega_D^2} \sin^2(\omega_D t) \cos(\omega_{\text{rf}} t) \, .
    \end{align*}
    The first term has equal-amplitude sidebands at $\pm(\omega_D \pm q_D) = \pm $ and the second term has smaller amplitude sidebands at $\pm 2\omega_D$.
\end{itemize}
...
By tuning the probe to a magic-zero wavelength at $\lambda = 790.0\unit{nm}$ \note{We used a wavelength of $\lambda=781.15\unit{nm}$ due to SNR concerns from RF/noise pickup, we can sight the Faraday paper for our apparatus and spectrograms?}, and ensuring it is linearly polarized, the probe exerts no scalar or vector light shift on the atoms.
...
We acquire this spectrum in a single shot (or two shots accounting for the field calibration, but that would require $\sim 2\times$ more traditional shots also), i.e. $20\unit{s}$.
