...
\begin{itemize}
    ...
    In electronically/magnetically sensitive spin experiments, magnetic field noise manifest as small perturbations $\Delta \hat{F}_{z}$ which shift the energy eigenvalues.
    Continuous measurement of spin systems using the Faraday effect necessitate that your state is magnetically sensitive $\langle \hat{F} \rangle \neq 0$, but a protected dark state from magnetic field fluctuations must satisfy $\hat{F}_{z}\ket{\psi} = \hat{0}$ \cite{aharon_general_2013}. 
    Therefore, no state exists that is both insensitive to magnetic fields fluctuations and capable of generating a continuous Faraday signal, the two are not mutually exclusive.
