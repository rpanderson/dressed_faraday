...
Viewed as an ac magnetometer, this information not only measures fields oscillating at the dynamically-tunable Rabi frequency, but self-certifies both its band centre and residual detuning error, in real time, while remaining fourth-order decoupled from slow field noise.
More broadly, the cyclic coupling we observe is sought-after for emulation of quantum spin ladders with frustrated interactions~\cite{??}.
...
In finalising this work we became aware of measurements of the same rf-dressed system by Dimitris et al~\cite{dimiris} using Rabi spectroscopy and projective readout. 
