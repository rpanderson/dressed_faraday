...
The amplitude of each sideband is proportional to the corresponding dressed-state coherence $\rho_{ij} = c_i^* c_j $, and the non-vanishing dressed state coupling(s) $\bra{i} \hat{F}_{x,y,z} \ket{j}$, as summarised for near-resonant coupling ($\abs{\Delta} \ll \Omega$) in Table~\ref{tab:sidebands}.
\begin{table*}[t]
...
Conventional decoupling ($q_R=0$) has variation $(\sqrt{5}-2)/2 \approx 0.118$ across this range, while the undressed Zeeman transitions in the low-field limit have normalized variation 0.5.
