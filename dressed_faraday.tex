...
% MJK: The optimum dynamically decoupled case would coincide with the longest decoherence time, this seems to be the trend in the literature.
...
% RPA: omitted 'merely' for reasons I can answer in person!
...
In the dressed system this means relatively low-frequency (`ac') fields oscillating near the Rabi frequency $\Omega$, such as $B_{y,\text{ac}} \vect{e}_y \cos \Omega t$ and $B_{z,\text{ac}} \vect{e}_z \cos \Omega t$, drive transitions $\ket{m_x=-1} \leftrightarrow \ket{m_x=0}$ and $\ket{m_x=0} \leftrightarrow \ket{m_x=+1}$.
...
\end{figure}
This yields dressed eigenstates $\{\ket{1}, \ket{2}, \ket{3}\}$ that are no longer $\text{SO}(3)$ rotations of the bare Zeeman states $\ket{m_z}$, but instead all have nematic order, and an eigenspectrum $\omega_i(\Delta) = E_{\ket{i}}/\hbar$ shown in \reffig{fig:eigensystem_schematic} (left).
% for $q$ between  [0, \Omega]$.
Moreover, the couplings between these dressed states are markedly different: the spin operators $\hat{F}_{x,y,z}$ represented in the dressed basis span more of $\text{SU}(3)$ for $q \neq 0$.
% LDT: Susbcript for dressed operators?
% RPA: The operators are the same, it's the matrix representation that's different. 
...
giving the subspace comprised of ${\ket{1},\ket{2}}$ a higher-order decoupling than can be achieved with a two-level system; we join Lundblad \etal in terming these `synthetic clock states'.
% LDT: ... or similar bombastic sentence. This needs oomph.
% LDT: Need to decide on spin-1/2 versus two-level etc.
...
% RPA: Out of order/preempts Fig. 2!
...
% RPA: No chaudhury_continuous_2006? It's very much related..
...
A periodogram (\reffig{fig:static_coupling}, right) of the entire time-series yields maximum frequency resolution (at the expense of all temporal resolution) and shows the outwardly-skewed peaks expected for transitions convex in $\Delta$, e.g. the $\omega_{23}$ sidebands have a substantial third moments and a second moment (standard deviation) that is $4$ times broader than the transform-limited peak of the $\omega_{12}$ transition ($\unit[24]{Hz}$ second-moment).
% LDT: Are we sure units of third moments are Hz not Hz$^3$?
% LDT: No, transform limited should be a few Hz only! 
\begin{figure}
...
% LDT: Which is relevant to us? How do I know?
% RPA: The projection(s) that give rise to the transition |i> <--> |j>.
\begin{table*}[t]
    ...
    % LDT: Note removal of curlies and fracbracks in cols 4 and 5 resp of table to reduce symbology. Debatable!
    \label{tab:sidebands}
    }
    \begin{ruledtabular}
    \begin{tabular}{ccccc}
    ...
    \end{ruledtabular}
\end{table*}
% RPA: The second column shouldn't have \pm as these are explicitly upper sidebands as per table caption and subsequent columns.
...
% RPA: Process tomography / Hamiltonian learning
...
% LDT: I don't like `amounts to'.
% RPA: You must since *you* added it twice here!
...
We nominally observe a three-fold increase in the lifetime of the spectral components corresponding to the $\omega_{12}$ and $\omega_{23}$ transitions as compared with the undressed system (\reffig{fig:acquisition_pipeline}a, $1/e$ decay time \unit[23.8(2)]{ms}).
...
To appraise the success of decoupling of the dressed state transitions, and their dependence on the quadratic shift $q_R$, we varied the magnetic field over a wider range than was furnished by our power line noise of \reffig{fig:static_coupling}, by sweeping the bias magnetic field along $z$ over a range $B_{\text{rf}}$ during the measurement interval.
% LDT: What is this $B_{rf}!?$
% RPA: Amplitude of the dressing field (Rabi frequency in magnetic field units). See page 1.
% RPA: Furnish is no longer on the swears list?
% MJK: We should tread lightly here on this previous comment. We should avoid drawing attention to the question ``If it is optimally decoupled, then it should show the longest decoherence time. Have you done this?''
The control field $B_z(t) = B_0 + \alpha t + B_{\text{line}}(t)$, where $\alpha = \unit[128]{mG/s}$ is the linear sweep rate; the resulting detuning variation is of order $2\Omega$ (\textit{cf.} the domain of~\reffig{fig:eigensystem_schematic}).
...
% RPA: *resonant* magnetic fields $B_0$ is essential syntax and 'bias' is jargon.
...
% LDT: No need for $|\cdot|$ as we always swept up, right?
% RPA: Needed 
% RPA: (angular freq) / (2pi) = X Hz is our standard (and more correct) convention.
...
The results of this model independent analysis are shown in~\reffig{fig:curvature_vs_qR} for $q_R$ between 0.2 and 0.5.
...
For example, the single spectrum shown in \reffig{fig:acquisition_pipeline} would take $\sim (10$ shots per $\delta B_z$ per $\omega_{ij} ) \times (100$ $\delta B_z$ values) $\times (3 $ transitions $\omega_{ij}) = 3000$ shots, or $\sim \unit[6\times10^4]{s} = 1000$ minutes of data acquisition.
% RPA: $2.5$ times fewer $\delta B_z$ values than shown here.
We acquire this spectrum in a single shot, i.e. $\unit[20]{s}$.
% RPA: Two shots accounting for the field calibration, but that would require $\sim 2\times$ more traditional shots.
