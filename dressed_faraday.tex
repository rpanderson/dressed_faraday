...
The decoupled microscale quantum gas offers magnetic sensitivity in a tunable kilohertz band, persistent over many milliseconds: this is the length scale, frequency, and duration relevant to many applications including sensing biomagnetic phenomena such as neural spike trains.
\end{abstract}
...
\begin{figure}
    ...
    \label{fig:curvature_vs_qR}
        ...
        The measured curvature (black points) is determined from polynomial fitting to $(\delta B_z, f_{12})$ data, e.g. \reffig{fig:acquisition_pipeline}(c).
...
In finalizing this work we became aware of measurements of the same rf-dressed system by Trypogeorgos \etal~\cite{dimitris_synthetic_2017} using projective readout of dressed state populations. 
