...
We continuously observe dynamical decoupling in a spin-1 quantum gas using a  weak optical measurement to `listen' to the precessing spin.
% Continuous measurement + rf dressing = new type of signal
Radiofrequency (rf) drive transforms a quavering Larmor tone into a stable `chord' of three notes.
% What the measurement tells us: dressed spectrum and friends
Spectrogram analysis parses these notes as splittings, coherences and coupling strengths between the rf-dressed eigenstates, continuously in a single preparation of the quantum gas.
% What we do with the measurement: explore quartic decoupling
We locate a quadratic Zeeman shift that isolates one of dressed state transitions from magnetic field fluctuations up to fourth order.
...
Marrying clock-state metrology with continuous measurement provides a platform for new magnetometry protocols and emulating quantum magnetism in solid-state systems.
\end{abstract}
