...
\begin{figure}
    ...
    Magnetic field fluctuations $\delta B_z(t)$ of amplitude $\unit[1.4]{mG}$ induced by mains power are manifest as asymmetric frequency modulation of the $\omega_{13}$ and $\omega_{23}$ sidebands, while the $\omega_{12}$ transition remains relatively unaffected.
    The corresponding spectral peaks have linewidths $\unit[xx]{Hz}$, $\unit[126]{Hz}$, and $\unit[36]{Hz}$, respectively.
    The $\omega_{12}$ linewidth is transform-limited, whereas the broadened peaks of the less decoupled $\omega_{23}$ transition exhibit a skew (third-moment) of $\unit[115]{Hz}$ (upper sideband) and $-\unit[137]{Hz}$ (lower sideband).
    }
\end{figure}
...
No attempt was made to shield the apparatus from parasitic magnetic fields, and mains power induced magnetic field noise causes a temporally varying $\delta B_z(t)$ at the line frequency of $\unit[50]{Hz}$ and odd harmonics of amplitude $\sim\unit[1.4]{mG}$ ($\unit[1]{kHz}$ in frequency units of detuning).
...
This is signified by a skew (third-moment) of e.g. $\unit[115]{Hz}$ and $-\unit[137]{Hz}$ of the upper and lower sidebands of the $\omega_{23}$ peaks in the periodogram (\reffig{fig:static_coupling}, right), and a $3.5\times$ wider linewidth than the transform-limited peak of the $\omega_{12}$ transition, which has a $\unit[36]{Hz}$ second-moment.
\begin{figure}
...
\begin{table*}[t]
    ...
    For the initial state $\ket{\psi(t=0)}=\ket{m_z=-1}$, the sideband frequencies and amplitudes can be expressed concisely in terms of the dressed Larmor frequency $\omega_D$ and quadratic shift $q_D$. 
