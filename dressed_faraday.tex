...
The three Zeeman states of the lowest hyperfine ($F=1$) ground state are coupled using a radiofrequency field with $\Omega/(2\pi) \leq 100\unit{kHz}$, generated by a single-turn coil placed immediately atop the glass vacuum cell, fed by an amplified radiofrequency source generated using direct-digital synthesis.
...
We detect the Faraday rotation of the probe light using a shot-noise limited balanced polarimeter, with bandwidth up to $8\unit{MHz}$, and record the signal using an AlazarTech \textsc{ATS9462} ditigizer ($16$-bit, $180\unit{MS/s}$)
...
Upon applying the radiofrequency (rf) dressing field, $\abs{\expect{F_x}} > 0$ and the signature of the coupled spin-1 system is a Faraday signal frequency modulated (FM) about a carrier at the Larmor frequency.
...
The time-dependent magnetic field shift $\Delta B(t)$ is the sum of an applied linear ramp and the parasitic background fluctuations at the power-line frequency of $50\unit{Hz}$ and its odd harmonics.
For each realization (or `shot') of the experiment, we directly calibrate this time-dependent field using ac magnetometry; an rf $\pi/2$-pulse (rather than continuous coupling) initiates Larmor precession of the collective spin the $x$--$y$ plane, and the Faraday signal is composed of two tones, at $\omega_L \pm q$.
The experiment is synchronized to the AC power line; the harmonic composition of which varies little between contiguous shots ($20\unit{s}$ apart), and thus the measured $\Delta B(t)$ and $q$ from the calibration shot serve as a good proxy for that experienced by the atoms in the subsequent rf-dressed shot.
