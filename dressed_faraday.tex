...
\begin{abstract}
% What did we do? Fewest words that include CoDD, spin-1, quantum gas, weak optical probe
We continuously observe a dynamically decoupled spin-1 quantum gas using a minimally perturbative atom-light interface.
% Continuous measurement + rf dressing = new type of signal
This weak measurement `listens' to the precessing spin, which under constant radiofrequency (rf) drive transforms from a quavering Larmor tone to a stable `chord' of three notes modulated onto a carrier. 
% What the measurement tells us: dressed spectrum and friends
Spectrogram analysis of the recording quantifies the splittings, coherences and coupling strengths between the three rf-dressed eigenstates, continuously in a single preparation of the quantum gas.
% What we do with the measurement: explore quartic decoupling
We vary the ratio of quadratic Zeeman shift to rf amplitude, locating where one of the three dressed state transitions is isolated from magnetic field fluctuations up to fourth order.
...
% Navel-gazing alternative
% This system weds the useful hallmarks of clock-state metrology with continuous measurement, a platform for new magnetometry protocols and emulating quantum magnetism in solid-state systems.
\end{abstract}
...
% Unless you're driving transitions populations don't tell you about couplings. Fx ``is'' driving transitions. 
%Fourier transform spectroscopy using projective measurements of dressed state populations tells you about the spectrum
...
In this regime, a synthetic clock transition arises which is only quartically-sensitive to noise, surviving much larger amplitude fluctuations than conventional quadratic decoupling.
