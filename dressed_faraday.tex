...
This protected \note{We shouldn't use protected since it is not strictly protected.} subspace is spanned by two of the dressed states, termed `synthetic clock states' in a co-submission by Lundblad~\etal (\lundblad).
...
%perpendicular to the quantization direction $z$,
...
\end{figure}
This yields dressed eigenstates $\{\ket{1}, \ket{2}, \ket{3}\}$ that are no longer $\text{SO}(3)$ rotations of the bare Zeeman states $\ket{m_z}$, but have nematic order~\note{LDT: \ket{m_z=0} is nematic!}, and an eigenspectrum $\omega_i(\Delta) = E_{\ket{i}}/\hbar$ shown in \reffig{fig:eigensystem_schematic} (left).% for $q$ between  [0, \Omega]$.
Moreover, the couplings between these dressed states are markedly different: the spin operators $\hat{F}_{x,y,z}$~\note{Susbcript for dressed operators?} in the dressed basis span more of $\text{SU}(3)$ for $q \neq 0$.
...
%$\bra{i} \hat{F}_x \ket{j}$ has a non-zero projection onto the fourth Gell-Mann matrix $Q_{x^2-y^2}$ \note{why is this the fourth Gell-mann matrix? $\lambda_{4}$ would be more common} signifying the direct coupling of dressed states $\ket{1}$ and $\ket{3}$.
Just as for bare states, we can characterize the splittings~\note{we are using splittings and transitions around here...} between dressed states near $\Delta=0$ as arising from a dressed Larmor frequency $\omega_D\equiv(\omega_3-\omega_1)/2=\sqrt{\Omega^2+q_D^2}$, and dressed quadratic shift $q_D \equiv (\omega_3 + \omega_1 -2\omega_2)/2=-q/2$, giving splittings $\omega_{23}=\omega_D-q_D$, $\omega_{12}=\omega_D+q_D$ and $\omega_{13}=2\omega_D$. 
...
The total magnetic field in the laboratory frame used to affect this control is $\vect{B}(t \geq 0) = B_z(t) \vect{e}_z - B_{\text{rf}} \vect{e}_x \cos \omega_{\text{rf}} t  $, where $B_z(t)$ varies very little over a Rabi flopping time $1/\Omega$.
The resulting Faraday signal $\expect{\hat{F}_x}(t)$ is presented in the time-frequency domain as a spectrogram, the magnitude of the short-time Fourier transform, revealing multiple sidebands with varying amplitudes and frequencies.
These directly measure the dressed state energies, coherences, and coupling strengths as we now describe.

For example, with no deliberate variation of the Rabi frequency or detuning, we observe the broadly stationary spectrogram shown in \reffig{fig:static_coupling}.
Strong amplitude modulation of the Faraday signal is apparent as three pairs of upper and lower sidebands, each pair equidistant from the carrier frequency at $f_{\text{rf}} = \omega_{\text{rf}}/(2\pi)$. 
\begin{figure}
...
A periodogram (\reffig{fig:static_coupling}, right) of the entire time-series yields maximum frequency resolution (at the expense of all temporal resolution) shows the outwardly-skewed peaks expected for transitions convex in $\Delta$, e.g. the $\omega_{23}$ sidebands have a substantial third moments
%of $\unit[84]{Hz}$ (upper) and $-\unit[100]{Hz}$ (lower),
\note{Are we sure units of third moments are Hz not Hz$^3$?}
and a second moment (standard deviation) that is $4$ times broader than the transform-limited~\note{no, transform limited should be a few Hz only!} peak of the $\omega_{12}$ transition ($\unit[24]{Hz}$ second-moment).
\begin{figure}
...
\begin{table*}[t]
    ...
    \note{Note removal of curlies and fracbracks in cols 4 and 5 resp of table to reduce symbology. Debatable!}
    \label{tab:sidebands}
    }
    \begin{ruledtabular}
    \begin{tabular}{ccccc}
...
Alternatively, if the dressed state couplings are separately characterized~\cite{lundblad_synthetic_2017}, this amounts to  real-time measurement of the dressed density matrix, effecting quantum state estimation of the dressed system.~\note{I don't like `amounts to'}.

To appraise the success of decoupling of the dressed state transitions, and their dependence on the quadratic shift $q_R$, 
we varied the magnetic field over a wider range than was furnished by our power line noise of \reffig{fig:static_coupling}, by sweeping the bias magnetic field along $z$ over a range $B_{\text{rf}}$~\note{What is this $B_{rf}!?$} during the measurement interval.
...
\note{If using $\Gamma$ must say that $\Gamma\gg 1$ means adiabatic or whatever...}
